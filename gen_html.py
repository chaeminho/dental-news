...
if latest_date:
    p = latest_date.split('-')
    latest_heading = f'{p[0]}년 {p[1]}월 {p[2]}일 뉴스'
...
EXT_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>'
CSS = """
...
main{max-width:1080px;margin:32px auto 48px;padding:0 16px;overflow:visible}
...
.card-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:18px;overflow:visible;align-items:start}
.card{background:#fff;border-radius:var(--r);padding:22px 22px 18px;box-shadow:var(--sh);border:1px solid rgba(200,220,240,.5);transition:transform .2s,box-shadow .2s;display:flex;flex-direction:column;gap:12px;height:auto}
...
"""
JS = f"""var ARTS={aj};var DATES={dj};var SO={soj};var EXT='{EXT_SVG}';
...
html_parts = [
    ...
    '<title>\U0001f9b7 미르치과 뉴스 데일리</title>',
    '<style>'+CSS+'</style></head><body>',
    '<header>',
    '<span class="logo">\U0001f9b7</span>',
    '<h1>미르치과 뉴스 데일리</h1>',
    '<p class="subtitle">매일 자동으로 업데이트되는 치과 업계 뉴스, 기술 트렌드, 건강보험 정책, SNS 마케팅 트렌드 모음</p>',
    f'<p class="date-heading" id="dh">{latest_heading}</p>',
    '<div class="filter-bar"><input type="date" id="dp"/><button id="ba">오늘 뉴스</button></div>',
    '</header>',
    '<main id="mc"></main>',
...
print(f'최신 날짜: {latest_date}, 총 기사 수: {len(articles)}')
